...
// Order component to display order details and handle payment
const Order = () => {
  ...
  // Mutation hooks for paying and delivering order
  ...
  // PayPal script reducer to manage PayPal SDK loading
  ...
  // Effect to handle PayPal script loading and order payment
  useEffect(() => {
  ...
  return isLoading ? (
    // Display loader while fetching order details
    <Loader />
  ) : error ? (
    // Display error message if fetching order details fails
    <Messsage variant="danger">{error.data.message}</Messsage>
  ) : (
    // Render order details and payment options
    <div className="container flex flex-col ml-[10rem] md:flex-row">
      ...
      <div className="md:w-1/3">
        ...
        {!order.isPaid && (
          <div>
            {/* Display loading spinner if payment is pending */}
            {loadingPay && <Loader />}
            {isPending ? (
              <Loader />
            ) : (
        ...
        {/* Mark as delivered button for admin users */}
